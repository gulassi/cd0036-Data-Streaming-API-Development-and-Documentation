...
encodedRedisMessageDF = spark.sql("select key, zSetEntries[0].element as redisEvent from RedisData")
...
decodedRedisMessageDF = encodedRedisMessageDF.withColumn("redisEvent", unbase64(encodedRedisMessageDF.redisEvent).cast("string"))

#TO-DO: repeat this a second time, so now you have two separate dataframes that contain redisEvent data
decodedRedisMessage2DF = encodedRedisMessageDF.withColumn("redisEvent", unbase64(encodedRedisMessageDF.redisEvent).cast("string"))
...
decodedRedisMessageDF.withColumn("redisEvent", from_json("redisEvent", customerSchema)).select(col("redisEvent.*")).createOrReplaceTempView("Customer")
...
decodedRedisMessageDF.withColumn("redisEvent", from_json("redisEvent", customerLocationSchema)).select(col("redisEvent.*")).createOrReplaceTempView("CustomerLocation")
...
customerDF = spark.sql("select accountNumber as customerAccountNumber, location as homeLocation, birthDay from Customer where birthDay is not null")

#TO-DO: select the customerAccountNumber, homeLocation, and birth year (using split)
customerParsedDF = customerDF.select("customerAccountNumber", "homeLocation", split(customerDF.birthDay, "-").getItem(0).alias("birthYear"))

#TO-DO: using spark.sql select accountNumber as locationAccountNumber, and location
customerLocationDF = spark.sql("select accountNumber as locationAccountNumber, location from CustomerLocation")

#TO-DO: join the customer and customer location data using the expression: customerAccountNumber = locationAccountNumber
customerInfoDF = customerParsedDF.join(customerLocationDF, expr("customerAccountNumber = locationAccountNumber"))
